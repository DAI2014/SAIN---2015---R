...
TS=read.table("hourly-unknown-4.ts",header=TRUE,sep=",")
TS=TS$ts # vector of numeric
mpause("show graph")
tsdisplay(TS)

L=length(TS)
H=21 # number of predictions

# time series monthly object:
TR=ts(TS,frequency=21,start=1,end=L-H)
...
print(F1)
Pred1=F1$mean[1:H]
mgraph(Target,Pred1,graph="REG",Grid=10,col=c("black","blue"),leg=list(pos="topleft",leg=c("target","predictions")))
cat("MAE:",mmetric(Target,Pred1,metric="MAE"),"\n")

# neural network modeling:
mpause("model NN (mlpe):")
d=CasesSeries(TS,c(21)) # data.frame from time series
LD=nrow(d)
